...
const Favorite = ({closeWishlist}:any) => {
  ...
  const lists = favoriteLists.map((i:Product) => {
    return(
        <div key={i._id} className="flex flex-col md:flex-row gap-2 border-b-4 p-2 shadow-lg border-gray-900 text-gray-900">
            <Image src={urlFor(i.image).url()}  width={100}
        height={100} className=" w-full md:w-1/4 h-44 object-cover" alt={i.name} />
        <div className=" flex flex-col justify-evenly">

        <Link href={i.slug.current} onClick={closeWishlist} className=" w-full block font-semibold text-xl hover:font-bold text-center">{i.name}</Link>
        <p>{i.description}</p>
